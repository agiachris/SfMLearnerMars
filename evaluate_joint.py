...
parser.add_argument('--run-sequence', type=str, required=True, help='run of the CEPT to evaluate on',
...
parser.add_argument('--skip-freq', default=10, type=int, help='sample frequency for visualization')
...
def main():
    ...
    test_set = CEPTDataset.CEPT(args.dataset_dir, 'val', args.sequence_length, args.seed)
    ...
    l_test, ate, ate_mean, gt_traj, pred_traj, rate = test(disp_net, pose_net, test_loader, criterion,
                                                           w_synth, w_smooth)
    total_time = time.time() - start_time
    visualizer.generate_random_visuals(disp_net, pose_net, test_loader, criterion,
                                       args.vis_per_epoch, epo, args.run_sequence, args.skip_freq)
    visualizer.generate_trajectories(gt_traj, pred_traj, epo, args.run_sequence)
    visualizer.generate_trajectory(gt_traj, 'gt', 'Ground Truth', epo, args.run_sequence)
    visualizer.generate_trajectory(pred_traj, 'pred', 'Predicted', epo, args.run_sequence)

    print_str = "ATE - {:.3f} | view synth loss - {:.3f} | smooth loss - {:.3f}".format(ate_mean, l_test[1], l_test[2])
    ...
    np.savetxt(os.path.join(log_path, '{}_ate_mean.txt'.format(args.run_sequence)), np.array([ate_mean]))
    np.savetxt(os.path.join(log_path, '{}_ate.txt'.format(args.run_sequence)), ate)
...
def test(disp_net, pose_net, test_loader, criterion, w1, w2):
    ...
    return total_loss / i, ate, ate_mean, traj_gt, traj_pred, rate
